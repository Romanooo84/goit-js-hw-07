...
const markup = images.map(image => `<li><img class="gallery__link gallery__image" id='${image.description}' src="${image.preview}" alt="${image.alt}"></li>`).join('');
...
document.querySelector('.gallery').onclick = (event) => {
	...
	//wyswietla id tego elementu (id='${image.description}')
	const idElement = imgElement.id;
